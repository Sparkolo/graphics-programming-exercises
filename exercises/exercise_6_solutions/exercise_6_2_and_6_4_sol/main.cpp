...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        srlRenderer->render(vtsWing, wingRightBack, buffer, zBuffer);

        // right wing,
        // mirror in x
        glm::mat4 wingLeft = mvp * glm::scale(-1.f, 1.f, 1.f);
        srlRenderer->render(vtsWing, wingLeft, buffer, zBuffer);

        // right wing back,
        // half size + mirror in x -> move to the back
        glm::mat4 wingLeftBack = mvp * glm::translate(0.0f, -0.5f, 0.0f) * glm::scale(-.5f, .5f, .5f);
        srlRenderer->render(vtsWing, wingLeftBack, buffer, zBuffer);
